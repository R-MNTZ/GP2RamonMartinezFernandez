...
void MyShader::init(const std::string& filename)
{
	...
	for (unsigned int i = 0; i < NUMBoF_SHADERS; i++)
	{
	...
	glLinkProgram(program); //create executables that will gameSceneRun on the GPU shaders
	CheckShaderError(program, GL_LINK_STATUS, true, "Error: MyShader program linking failed"); // cheack for error

	glValidateProgram(program); //check the entire program is valid
	CheckShaderError(program, GL_VALIDATE_STATUS, true, "Error: MyShader program not valid");
